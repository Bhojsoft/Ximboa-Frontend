...
export class EditProfilePictureComponent implements OnInit {
  ...
  trainer_image: File | null = null;
  maxFileSizeMB: number = 2; // Limit file size to 2 MB
  allowedFileTypes: string[] = ['image/jpeg', 'image/png']; // Corrected allowed file types to include only the necessary types
  maxResolution = { width: 400, height: 400 }; // Maximum resolution set to 400x400 pixels
  uploadAttempts = 0;
  maxAttempts = 5;
  ...
  onFileSelected(event: any): void {
    ...
    if (file) {
      ...
      img.onload = () => {
        ...
        // Check image resolution: ensure it's not larger than 400x400 pixels
